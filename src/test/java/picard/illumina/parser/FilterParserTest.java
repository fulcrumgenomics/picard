...
public class FilterParserTest {
    ...
    public static final Boolean[] allTilesToValues(int[] tiles) {
    ...
    public void passingParserTest(int[] tiles) {
        final IlluminaFileUtil fUtil = new IlluminaFileUtil(BaseCallsDir, 1);

        final Boolean[] values = allTilesToValues(tiles);

        final FilterParser fp = new FilterParser(((PerTileFileUtil) fUtil.getUtil(IlluminaFileUtil.SupportedIlluminaFormat.Filter)).getFiles(tiles));
        fp.verifyData(tiles, null); //filter parser doesn't care about cycle
    ...
    public Object[][] seekToTile() {
        return new Object[][]{
                ...
                {0, 2, new int[]{2}, new Boolean[][]{s_1_0002_filter}},
                {4, 4, new int[]{1, 4}, new Boolean[][]{new Boolean[]{F, F, T, F}, s_1_0004_filter}},
                {0, 3, new int[]{2, 3}, new Boolean[][]{s_1_0003_filter}},
                {15, 3, new int[]{1, 2, 3, 4}, new Boolean[][]{s_1_0001_filter, s_1_0003_filter, s_1_0004_filter}}
        };
    }

    @Test(dataProvider = "seekToTile")
    public void passingSeekingParserTest(int skipBefore, int tileToSkipTo, int[] tiles, Boolean[][] expectedValues) {
        final IlluminaFileUtil fUtil = new IlluminaFileUtil(BaseCallsDir, 1);
    ...
    public Object[][] failingVerifyTiles() {
        return new Object[][]{
                ...
                {new int[]{2}, new int[]{1}},
                {new int[]{4}, new int[]{5}},
                {new int[]{1, 4}, new int[]{1, 3, 4}},
                {new int[]{2, 3}, new int[]{2, 3, 4}},
    ...
    public void verifyDataTest(final int[] initTiles, final int[] verifyTiles) {
        final IlluminaFileUtil fUtil = new IlluminaFileUtil(BaseCallsDir, 1);
        final FilterParser fp = new FilterParser(((PerTileFileUtil) fUtil.getUtil(IlluminaFileUtil.SupportedIlluminaFormat.Filter)).getFiles(initTiles));
        fp.verifyData(verifyTiles, null); //filter parser doesn't care about cycle values
        fp.close();
    }
}
